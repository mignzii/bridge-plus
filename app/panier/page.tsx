    ...
    <div className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <nav className="hidden md:flex space-x-8">
                <button onClick={()=>router.push('./commande')} className="text-gray-700 hover:text-red-600 font-medium">
                    Commander
                </button>
            </nav>
      ...
      <section>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-6 shadow-sm">
          <h1 className="text-2xl font-bold text-gray-900 mb-6">Panier</h1>
          
          <div className="space-y-4 border rounded-xl border-gray-200 ">
            {PanierItems.map((item) => (
              <div key={item.id} className="flex items-center gap-4 p-4 rounded-xl">
                <div className="w-20 h-20 bg-gradient-to-br from-orange-400 to-red-500 rounded-xl flex-shrink-0 overflow-hidden">
                  <div className="w-full h-full bg-gradient-to-br from-orange-200 to-orange-400 flex items-center justify-center">
                    <img src={item.image} alt={item.produit_nom} className='w-20 h-20' />
                  </div>
                </div>
                
                <div className="flex-1">
                  <h3 className="font-semibold text-gray-900 text-lg">{item.produit_nom}</h3>
                  <p className="text-sm text-gray-600">Accompagnement : {
                            item.accompagnant
                            ? JSON.parse(item.accompagnant)[0]  
                            : "Aucun"
                        }</p>
                  <p className="text-sm text-gray-500">Catégorie : {getCategorieNom(item.categorie_id)}</p>
                  <p className="font-bold text-lg text-gray-900 mt-1">FCFA {item.prix_unitaire}</p>
                </div>
                
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-4">
                    <button className=" px-5 py-1 rounded-full space-x-2 bg-gray-100 flex items-center justify-center hover:bg-gray-50 transition-colors">
                    <Minus onClick={()=>decrementQuantity(item.id)} className="w-4 h-4" />
                    <span className="text-lg font-semibold min-w-[2rem] text-center">
                    {quantities[item.id] ?? 1}
                    </span>
                    <Plus onClick={()=>incrementQuantity(item.id)} className="w-4 h-4" />
                    </button>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm h-fit border border-gray-200 ">
          ...
          {/* Payment Button */}
          <button onClick={()=>router.push("./paiement")} className="w-full bg-black text-white py-2 rounded-full font-semibold text-lg hover:bg-gray-800 transition-colors flex items-center justify-center gap-2">
            <span>Paiement </span>
            <ArrowRight size={20} />
          </button>
        </div>
      ...
      <section className="py-6 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-8">Articles en vedettes</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
           ...
           <h2 className="text-2xl font-bold text-gray-900 mb-8">Offre du jours</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
