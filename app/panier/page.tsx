...
const clearCartHandler = async () => {
 ...
 useEffect(() => {
  ...
  {PanierItems.map((item) => (
    ...
    >
      {/* Image */}
      <div className="w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-br from-orange-400 to-red-500 rounded-xl flex-shrink-0 overflow-hidden">
      ...
      <div className="flex-1">
        ...
        <p className="text-sm text-gray-600">
          Accompagnement :{" "}
          {item.accompagnant ? JSON.parse(item.accompagnant)[0] : "Aucun"}
        </p>
        <p className="text-sm text-gray-500">
          Catégorie : {getCategorieNom(item.categorie_id)}
        </p>
        <p className="font-bold text-base sm:text-lg text-gray-900 mt-1">
          FCFA {item.prix_unitaire}
        </p>
      </div>

      {/* Actions */}
      <div className="flex sm:flex-col md:flex-row items-center gap-3 sm:ml-auto">
        {/* Ajout / Suppression */}
